...
public class CurveScene2D extends Scene2D {
    ...
    /**
     * Display settings: convex hull (not suitable for Hermite curve)
     */
    ...
    /**
     * Display settings: control polygon (not suitable for Hermite curve)
     */
    ...
    public JPanel getUserInterface() {
        ...
        // Control points
        JCheckBox checkBoxControlPoints = new JCheckBox("Show control points");
    ...
    protected int getClosestControlPointIndex(int x, int y) {
        ...
        for (int i = 0; i <= curve.getDegree(); i++) {
            Vector2f cp = curve.getControlPoint(i);
    ...
    private void drawCurve(Graphics gc) {
        ...
        if (showControlPoints) {
            ...
            if (curve.getType() == BasisFunction.CurveType.BEZIER) {
                for (int i = 0; i <= curve.getDegree(); i++) {
                    drawPoint(gc, curve.getControlPoint(i), CONTROL_POINT_COLOR);
                }
            } else if (curve.getType() == BasisFunction.CurveType.HERMITE) {
                // Start- and Endpoint
                drawPoint(gc, curve.getControlPoint(0), CONTROL_POINT_COLOR);
                drawPoint(gc, curve.getControlPoint(3), CONTROL_POINT_COLOR);
                drawLine(gc,
                        curve.getControlPoint(0),
                        curve.getControlPoint(0).add(curve.getControlPoint(1)),
                        CONTROL_POINT_COLOR);
                drawLine(gc,
                        curve.getControlPoint(3),
                        curve.getControlPoint(3).add(curve.getControlPoint(2)),
                        CONTROL_POINT_COLOR);
            }
        }
